...
async function handleFile(
  ...
  const enMessages = await getTranslations(
    'en',
    lokaliseToken,
    LOCALISE_PROJECT_ID,
  )
  ...
  for (const lang of languagesArray) {
    const prismicLocale = locales.find(
    ...
    for (const file of fileNames) {
      ...
      if (filterArray.includes(fileName) || !filter) {
        ...
        const fileGroupId = infoForFiles.find(
          ({ fullFileName }: { fullFileName: any }) => fullFileName === file,
        )?.groupId
        console.log('fileGroupId', fileGroupId)
        const translatedFileName = infoForFiles.find(
          ({ groupId, locale }: { groupId: any; locale: any }) =>
            fileGroupId === groupId && prismicLocale === locale,
        )?.fullFileName
        console.log(
          'translatedFileName',
          translatedFileName,
  ...
  zipWrite.generateAsync({ type: 'blob' }).then(
    function (blob) {
      console.log('saveAs')
...
export default function ApplyRecentTranslations() {
  ...
  const { register, handleSubmit, watch, formState, setValue, trigger } = form
  const onSubmit = (data: any) => {
    const { prismicZipFile, filter, languages, lokaliseToken } =
      form.getValues()
    for (var i = 0; i < prismicZipFile.length; i++) {
      handleFile(
        prismicZipFile[i],
        filter,
        ['de', 'es', 'sv', 'da', 'nl', 'fr', 'fi', 'it', 'pt', 'no'],
        lokaliseToken,
      )
    }
  }
